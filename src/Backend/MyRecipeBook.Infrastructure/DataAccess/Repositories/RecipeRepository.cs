...
namespace MyRecipeBook.Infrastructure.DataAccess.Repositories
{
    public sealed class RecipeRepository : IRecipeWriteOnlyRepository, IRecipeReadOnlyRepository, IRecipeUpdateOnlyRepository
    {
        ...
        public async Task<IList<Recipe>> Filter(User user, FilterRecipesDto filters)
        {
            ...
            {
                query.Where(recipe => recipe.Difficulty.HasValue && filters.Difficulties.Contains(recipe.Difficulty.Value));
            }

            if (filters.CookingTimes.Any())
            {
                query.Where(recipe => recipe.CookingTime.HasValue && filters.CookingTimes.Contains(recipe.CookingTime.Value));
            }
